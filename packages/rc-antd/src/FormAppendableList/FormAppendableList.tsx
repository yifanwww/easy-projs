...
export function FormAppendableList<T>(props: FormAppendableListProps<T>) {
  ...
  const renderExtraItem = useCallback(
    (item: React.ReactNode) => {
      ...
      return ['string', 'number', 'boolean', 'undefined'].includes(typeof item) || item === null ? item : undefined;
    },
    [readonly],
  );
    ...
    (remove: FormListOperation['remove'], fieldName: number, fieldsLength: number) => {
      ...
      return (
        <Button
          type="text"
          disabled={
            !!disabled || !!getDeletable?.(fieldName, fieldsLength) || (!!disableDeleteFirst && fieldName === 0)
          }
