...
Write the body to a **unique** temp file under `.tmp/prs/`, then pass it with `--body-file`.

**Important:** Always use a unique filename to avoid collisions when creating multiple PRs
concurrently. Derive the filename from the PR title by slugifying it — lowercase, hyphens for spaces,
strip special characters. Example: for the title "feat(settings): add dark mode toggle", use
`.tmp/prs/feat-settings-add-dark-mode-toggle.md`.

```bash
gh pr create \
  --title "feat(<scope>): <concise description>" \
  --body-file .tmp/prs/<slugified-title>.md
```
