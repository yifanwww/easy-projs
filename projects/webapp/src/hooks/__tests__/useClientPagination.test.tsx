...
describe(`Test fn \`${mutateClientPagination.name}\``, () => {
    ...
    it('should update URLSearchParams (functional updates)', () => {
        ...
        mutateClientPagination(usp, (prev) => ({ page: prev.page + 1 }));
        expect(usp.toString()).toBe('page=3');

        mutateClientPagination(usp, (prev) => ({ ...prev, pageSize: 20 }));
        expect(usp.toString()).toBe('page=3&page_size=20');

        mutateClientPagination(usp, (prev) => ({ ...prev, pageSize: 30 }));
        mutateClientPagination(usp, (prev) => ({ ...prev, page: 10 }));
...
describe(`Test react hook \`${useClientPagination.name}\``, () => {
    ...
    it('should read and update the client pagination', () => {
        ...
        expect(result.current[0]).toStrictEqual({ page: 3, pageSize: 10 });

        act(() => {
            result.current[1]({ pageSize: 20 });
        });
        expect(result.current[0]).toStrictEqual({ page: 1, pageSize: 20 });

        act(() => {
            result.current[1]({ page: 5 });
            result.current[1]({ pageSize: 50 });
        });
        expect(result.current[0]).toStrictEqual({ page: 1, pageSize: 50 });
    });
    ...
    it('should update the client pagination (functional updates)', () => {
        ...
        act(() => {
            result.current[1]((prev) => ({ ...prev, pageSize: 20 }));
        });
        ...
        act(() => {
            result.current[1]((prev) => ({ ...prev, pageSize: 30 }));
            result.current[1]((prev) => ({ ...prev, page: 10 }));
        });
